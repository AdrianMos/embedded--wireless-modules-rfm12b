...
int DATA_receive_and_decode(u8 *out)
{
...
if(rec!=TIMEOUTLOOP_EXPIRAT)  //daca s-au receptionat date
{
   DATA_decode(  rec,  &out1); //receptioneaza si decodeaza 8 biti

   rec=RF_receive();
  
   if(rec!=TIMEOUTLOOP_EXPIRAT) //daca s-au receptionat date
   {
  		DATA_decode(  rec,  &out2); //citeste si decodeaza 8 biti
		...
		else return DATA_EROARE_DUBLA;  //eroare dubla
  }
  else return TIMEOUTLOOP_EXPIRAT;  //iesire prin timeout de la RF_receive
}
else return TIMEOUTLOOP_EXPIRAT;   //iesire prin timeout de la RF_receive
