    ...
    //!= DATA_EROARE_DUBLA)
    {
        ...
        if (header == GLOBAL_DEST_ADDR(SYSTEM_ADDR, CURRENT_ADDR))
        {
            if (dat=='X')      //daca a receptionat cerere transmite informatia de raspuns
            {
                ...
                if ((GET_BIT(PINC, 1)==0) && (GET_BIT(PINC, 1)==0))   
                    DATA_code_and_send('D');
                else if(GET_BIT(PINC,  2)==0)                         
                    DATA_code_and_send('A');
                else if (GET_BIT(PINC, 1)==0)                         
                    DATA_code_and_send('B');
                else if (GET_BIT(PINC, 0)==0)                         
                    DATA_code_and_send('C');
                else                                                   
                    DATA_code_and_send(0);
