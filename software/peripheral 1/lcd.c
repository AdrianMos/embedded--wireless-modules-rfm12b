...
//void LCD_wait();  to be implemented next time & replace the LCD_delay
//u8 LCD_read ();   - used in LCD_wait
...
void LCD_wait()
{
    ...
    //MODE: 	Read busy flag (DB7) and address counter (DB0 to DB6)
	CLR_BIT(RS_PORT, RS);  //command
	SET_BIT(RW_PORT, RW);  //write to lcd

	DB_AS_INPUTS();
	do
	{
		//use these lines or implement LCD_read();
		SET_BIT(EN_PORT, EN);
		read = GET_BIT(DB_PORT_READ, DB7); //read the higher nibble
		CLR_BIT(EN_PORT, EN);
    
		SET_BIT(EN_PORT, EN);              //read the lower nibble
		CLR_BIT(EN_PORT, EN);

	} while (read==1); //bit 7 high => LCD still bussy

	DB_AS_OUTPUTS();
	CLR_BIT(RW_PORT, RW);          //RW=0 -> write to LCD
...
u8 LCD_read()
{
	...
	value = ( GET_BIT(DB_PORT_READ, DB4)<<4 )             //higher nibble
    	    | (GET_BIT(DB_PORT_READ,DB5)<<5)
			 | (GET_BIT(DB_PORT_READ,DB5)<<6)
			| (GET_BIT(DB_PORT_READ,DB7)<<7);
	...
	value =value
			 | GET_BIT(DB_PORT_READ, DB4)               //lower nibble
        	| (GET_BIT(DB_PORT_READ,DB5)<<1)
		 	 | (GET_BIT(DB_PORT_READ,DB6)<<2)
			| (GET_BIT(DB_PORT_READ,DB7)<<3);
